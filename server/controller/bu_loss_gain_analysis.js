const lossGainSrvc = require('../services/bu_loss_gain_analysis')
...
const insertOne = async (req, res) => {
    try {
        const result = await lossGainSrvc.save(req.body)
        res.send(result)
    } catch (err) {
        res.status(500).send({
...
const insertMany = async (req, res) => {
    try {
        const result = await lossGainSrvc.saveBulk(req.body)
        res.send(result)
    } catch (err) {
        console.log(err)
        res.status(500).send({
...
const getDataForLossGain = async (req, res) => {
    try {
        ...
        // console.log(query)
        const result = await lossGainSrvc.sortAndFetch(query, { sort: { date: 1 } })
        res.status(200).send({
            success: true,
            count: result.length,
            data: result
...
const fetchAll = async (req, res) => {
    try {
        const result = await lossGainSrvc.fetch({})
...
const migrate = async (req, res) => {
    try {
        let result = []
        axios.get(`${URL}/api/bpcl/business_unit/loss_gain/fetch/all`)
            .then(async response => {
                result = response.data
                console.log("fetched data :", result.length)
                // create json file in mongo_data folder
                await utility.writeFile('./mongo_data/bu_loss_gain_analysis.json', JSON.stringify(result))
                // insert all data in db
                const insertedData = await lossGainSrvc.saveBulk(result)
