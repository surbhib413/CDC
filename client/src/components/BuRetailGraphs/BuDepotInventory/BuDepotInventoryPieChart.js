...
import styles from "./BuDepotInventoryPieChart.module.scss";
  ...
  useEffect(() => {
    ...
    function prepareGraphData(apiGraphData) {
      apiGraphData.forEach((item, index) => {
        actualArray.push(item.meets_target);
        actualArray.push(item.below_target);
        actualArray.push(item.above_target);
      });
    }
