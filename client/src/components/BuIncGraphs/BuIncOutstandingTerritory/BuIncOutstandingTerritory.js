...
function BuIncOutstandingTerritory(props) {
    ...
    const prepareGraphData = () => {
        let url = `${environment.BU_KPI_URL}/payment/data?data_type=ytm&level=country&business_unit=inc&kpi_name=outstanding territory wise`;
        axios.get(url)
            .then(response => {
    ...
    return (
        <div className={`${styles.BuLubesLossGainAnalysisContiner}`}>
            <div className={`pt-2 ${styles.headerContainer}`}>
                <div className={`row`}>
                    <div className={`col-8`}>
                        <span>Outstanding Territory Wise</span>
                    </div>
                    {
                        lastItem ?
                                ...
                                graphData.map((item, index) => {
                                    return (
                                        <div key={index} className={`d-flex ${styles.tableDataRow}`}>
                                            <div className={`${styles.dataCell}`}><span>{item.not_due + item.zero_to_three_months + item.three_to_six_months + item.six_to_twelve_months + item.twelve_to_thirty_six_months + item.legal} Cr</span></div>
                                        </div>
                                    )
                                })
                            }
