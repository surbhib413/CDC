...
import styles from "./BuIncHsse.module.scss";
...
function BuIncHsse() {
    ...
    const [fatalitiesData, setFatalitiesData] = useState([]);

    const prepareData = () => {
        let url = `${environment.BU_KPI_URL}/others/data?data_type=ytm&level=country&business_unit=inc&kpi_type=hsse`;
        fetch(url, { method: "GET" })
            ...
            .then(response => {
                response.data.forEach((item, index) => {
                    ...
                    if (item.kpi_name === "Fatalities") { setFatalitiesData(item); }
                });
            })
            ...
            <div className={`${styles.contentContainer}`}>
                ...
                <div className={`d-flex ${styles.contentRow1}`}>
                    <div className={`d-flex flex-column justify-content-center ${styles.graphHeaderSection1}`}>
                        ...
                        <div className={`${styles.graphHeaderSectionBottom}`}><span>Pending Recommendation ESA</span></div>
                    </div>
                    <div className={`d-flex flex-column justify-content-center ${styles.graphHeaderSection1}`}>
                        <div className={`${styles.graphHeaderSectionTop}`}>
                        ...
                        <div className={`${styles.graphHeaderSectionBottom}`}><span>Pending Recommendation ISA</span></div>
                    </div>
                </div>
                <div className={`d-flex ${styles.contentRow1}`}>
                    <div className={`d-flex flex-column justify-content-center ${styles.graphHeaderSection2}`}>
                        <div className={`${styles.graphHeaderSectionTop}`}>
                        ...
                        <div className={`${styles.graphHeaderSectionBottom}`}><span>Accident Free Million Man Hours</span></div>
                    </div>
                    <div className={`d-flex flex-column justify-content-center ${styles.graphHeaderSection2}`}>
                        <div className={`${styles.graphHeaderSectionTop}`}>
                ...
                <div className={`${styles.barContainer}`}>
                    ...
                    <div className={`d-flex justify-content-end ${styles.fatalitiesText}`}>
                        <span>Fatalities: {fatalitiesData.actual}</span>
                        <span style={{ color: `${Math.sign(fatalitiesData.growth) === 1 ? "#74ce12" : "#e13219"}` }} className={`ml-3`}>{fatalitiesData.growth}%</span>
                    </div>
                </div>
            </div>
