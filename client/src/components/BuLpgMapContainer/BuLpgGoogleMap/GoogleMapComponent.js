...
class GoogleMapComponent extends Component {
  ...
  componentDidMount() {
    let url = `${environment.BU_KPI_URL}/overall/data?data_type=ytm&level=region&business_unit=lpg`;
      ...
      this.state.loading
        ...
        >
          <div className={`${styles.mapKpiSelectorContainer}`}>
            ...
            <div onClick={() => this.onKpiTabChange("supply")} className={`d-flex align-items-center justify-content-center flex-column ${styles.mapKpiSelector} ${this.state.activeKpi === "supply" ? styles.active : ""}`}>SUPPLY</div>
          </div>
          ...
          <BuLpgMapTable activeKpi={this.state.activeKpi}></BuLpgMapTable>
          {
            this.props.mapOf === "all regions" && this.props.activeTab === "lpg_tab"
              ?
              statesJson.regions.map((regionItem, index) => {
                ...
                return (
                  responseRegion && <React.Fragment key={index}>
                    <CustomInfoBox
